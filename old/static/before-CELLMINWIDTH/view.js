...
const CELLSETPOSITIONGAP    = 10;
const CELLMINWIDTH          = 25;
const CELLMINHEIGHT         = 25;
//const CELLMINSTYLE          = `box-sizing: border-box; min-width: ${CELLMINWIDTH}px; min-height: ${CELLMINHEIGHT}px;`;
const CELLMINSTYLE          = ``;
...
export class View extends Interface
{
 static style = { // Todo0 - Make new class .cellmarginoffset: { "margin-top": "-1px;", "margin-left": "-1px;" }
                  ...
                  ".itemmarginoffset":           { "margin-top": "-1px;", "margin-left": "-1px;", "box-sizing": "border-box;", "min-width": `${CELLMINWIDTH}px;`, "min-height": `${CELLMINHEIGHT}px;` },
      ...
      if (this.cursor.alignment.sides & ALIGNLEFT || this.cursor.alignment.sides & ALIGNRIGHT)
         {
          ...
          this.cursor.alignment.sides & ALIGNLEFT ? this.itemArea.x2 += itemwidth : this.itemArea.x1 -= itemwidth;
      ...
      if (this.cursor.alignment.sides & ALIGNTOP || this.cursor.alignment.sides & ALIGNBOTTOM)
         {
          ...
          this.cursor.alignment.sides & ALIGNTOP ? this.itemArea.y2 += itemheight : this.itemArea.y1 -= itemheight;
 ...
 DisplayGridContainer()
 {
  ...
  if (this.cursor.alignment)
     { 
      ...
      if (this.cursor.alignment.sides & ALIGNRIGHT) this.scrollingcontainer.scrollLeft =  item.offsetLeft + item.offsetWidth - (this.cursor.alignment.visiblearea.x2 - this.cursor.alignment.visiblearea.x1 + 1) + CELLSETPOSITIONGAP;
       else if (this.cursor.alignment.sides & ALIGNLEFT) this.scrollingcontainer.scrollLeft =  item.offsetLeft - CELLSETPOSITIONGAP;
      if (this.cursor.alignment.sides & ALIGNBOTTOM) this.scrollingcontainer.scrollTop =  item.offsetTop + item.offsetHeight - (this.cursor.alignment.visiblearea.y2 - this.cursor.alignment.visiblearea.y1 + 1) + CELLSETPOSITIONGAP;
       else if (this.cursor.alignment.sides & ALIGNTOP) this.scrollingcontainer.scrollTop =  item.offsetTop - CELLSETPOSITIONGAP;
 ...
 GetArea1SidesToExpandArea2(area1, area2)
 {
  ...
  if (area1.x2 < area2.x1 || area1.x1 > area2.x2 || area1.y2 < area2.y1 || area1.y1 > area2.y2) return; // Return undefined for two areas no intersection also
  ...
  for (let x = area.x1; x <= area.x2; x++)
      {
       ...
       if (!cell && (inner = inner + `<div class="undefinedcell itemmarginoffset${addclass}" id="${id}" style="${CELLMINSTYLE}"></div>`)) continue;
       const title = cell.hint ? ` title="${cell.hint}"` : ``;
       inner += `<div class="${cell.class} itemmarginoffset${addclass}" id="${id}" style="${cell.style} ${CELLMINSTYLE}"${title}>${cell.value}</div>`;
      }
 ...
 MoveCursor(offset, selection, resetareas)
 {
  ...
  if ('x' in offset) alignment.sides += offset.x > 0 ? ALIGNRIGHT : ALIGNLEFT;
  if ('y' in offset) alignment.sides += offset.y > 0 ? ALIGNBOTTOM : ALIGNTOP;
