...
export class View extends Interface
{
 static style = { // Todo0 - Make new class .cellmarginoffset: { "margin-top": "-1px;", "margin-left": "-1px;" }
		         ...
		         ".gridcontainer":              { "width": "100%;", "height": "100%;", "box-sizing": "border-box;", "overflow": "auto;", "border": "none;", "outline": "none;" },
		         ".grid":                       { "display": "grid;", "box-sizing": "border-box;", "overflow": "none;", "margin": "0px;", "padding": "1px 0 0 1px;" },
		         ".undefinedcell":              { "padding": "10px;", "border": "1px solid #999;", "background-color": "", "margin-top": "-1px;", "margin-left": "-1px;" },
		         ".titlecell":                  { "padding": "10px;", "border": "1px solid #999;", "color": "black;", "text-align": "center;", "background-color": "#CCC;", "font": "", "margin-top": "-1px;", "margin-left": "-1px;" },
		         ".newobjectcell":              { "padding": "10px;", "border": "1px solid #999;", "color": "black;", "text-align": "center;", "background-color": "#EFE;", "font": "", "margin-top": "-1px;", "margin-left": "-1px;" },
		         ".interactivecell":            { "padding": "10px;", "border": "1px solid #999;", "color": "black;", "text-align": "center;", "background-color": "",      "font": "12px/14px arial;", "margin-top": "-1px;", "margin-left": "-1px;" },
		         ".noninteractivecell":         { "padding": "10px;", "border": "1px solid #999;", "color": "black;", "text-align": "center;", "background-color": "#EEE;", "font": "12px/14px arial;", "margin-top": "-1px;", "margin-left": "-1px;" },
		         ".virtualcell":                { "padding": "10px;", "border": "1px solid #999;", "color": "black;", "text-align": "center;", "background-color": "#EEE;", "font": "12px/14px arial;", "margin-top": "-1px;", "margin-left": "-1px;" },
 ...
 DisplayView(errormsg)
 {
  ...
  for (let y = 0; y < this.valuetableHeight; y++)
      {
       if (!this.valuetable[y])
          {
           //for (let x = 1; x <= this.valuetableWidth; x++) inner += `<div class="undefinedcell" id="x${x}y${y}" style="grid-column=${x}; grid-row=${y};"></div>`;
           for (let x = 0; x < this.valuetableWidth; x++) inner += `<div class="undefinedcell" id="x${x}y${y}"></div>`;
           continue;
          }
       for (let x = 0; x < this.valuetableWidth; x++)
	       {
            const id = `x${x}y${y}`;
            const style = `grid-column=${x}; grid-row=${y};`;
            const cell = this.valuetable[y][x];
	        if (!cell && (inner = inner + `<div class="undefinedcell" id="${id}" style="${style}"></div>`)) continue;
            const title = cell.hint ? ` title="${cell.hint}"` : ``;
            inner += `<div class="${cell.class}" id="${id}" style="${cell.style} ${style}"${title}>${cell.value}</div>`;
           }
      }
  ...
  this.elementDOM.innerHTML = `<div class="gridcontainer"><div class="grid" style="${style}"${title}>${inner}</div></div>`;
  this.gridcontainer = this.elementDOM.querySelector('.gridcontainer');
  this.grid = this.gridcontainer.querySelector('.grid');
  this.props.control.mouseareaselect.elements.push([this.grid]);
 ...
 {
  return this.grid.querySelector(`#x${x}y${y}`);
 }
 ...
 MoveCursor(oldcursor, newcursor)
 {
  ...
  lg("cell.offsetTop", cell.offsetTop, this.gridcontainer.scrollTop, this.gridcontainer.offsetTop);

  //lg("grid.offsetTop", this.grid.offsetTop);
  lg("gridcontainer.offsetTop", this.gridcontainer.offsetTop);
 ...
 // 1. does nothing, plan rAF() to compute whether grid container is out of visible area (fully or partly)
 // Function calculates new render area x1, y1, x2 and y2 coordinates from viewcontainer.scrollLeft/viewcontainer.scrollTop till viewcontainer.scrollLeft + viewcontainer.clientWidth/viewcontainer.scrollTop + viewcontainer.clientHeight (not including edge values)
 // If new render area equals old render area (this.render.area) - do nothing (grid container will automatically be moving inside view container)
 // Otherwise get new area inner, 
 CalculateRenderArea()
 {
  const overlap = this.DoesAreaOneOverlapsAreaTwo(this.gridcontainer, this.viewcontainer);
  if (overlap) return;
