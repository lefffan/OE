...
export class Sidebar extends Interface
{
 ...
 //       [ {...} ],
 //      ]
 // this.od[id] { subtree:, ov{id}: {footnote: 0-..; status: -2||undefined (not open), -1 (server data pending), 0-100(view data loaded in %); DOMElements: [] } }
 // Add database (OD) or view (OV). View id (ovid) is undefined for OD
 // New tree concept:
 // this.tree = { type: folder/database/view, sort: , wrapped: true/false, name: , id: , odid: for the view type only, target: , content: [] } + search: , hierarchy: , activeodid: , activeovid: 
 // this.databases{id} = { subtree: , ov{id}: { footnote: 0-.., status: -2||undefined (not open); -1 (server data pending); 0-100(view data loaded in %), childid: , targets: [] } }
  ...
  for (const i in path)
      {
       ...
       if (type === 'view') this.od[odid]['ov'][ovid] === undefined ? this.od[odid]['ov'][ovid] = { footnote: '', subtree: [tree.at(-1)] } : this.od[odid]['ov'][ovid]['subtree'].push(tree.at(-1)); // The view doesn't exist. Add it to OD list
  ...
  switch (event.type)
	    {
	     ...
	     case 'mouseup':
               ...
               if (event.button === 0)
                  {
                   if (event.target.attributes['data-ovid'] !== undefined) // Old version: if (!subtree?.[0]) break; if (subtree[0].type === 'view') this.parentchild.CallController({ type: 'GETVIEW', data: { odid: subtree[0].odid, ovid: subtree[0].id } });
                      {
                       // Todo - Single OV click: OV is already open ? set OV active or refresh if already active : open in a current active view or in a new view if no any view exist
                   ...
                   if (!subtree?.[0] || subtree.length < 2) break;
