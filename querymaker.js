...
export class QueryMaker
{
 ...
 {
  return typeof prop === 'string' ? `${column}->'${prop}'` : `${column}->'${prop}'`;
 }
  ...
  for (const i in props.fields)
      {
       ...
       if (typeof field === 'string') props.fields[i] = { name: field };                                    // Bring string type field to object
       ...
       if (!field.escape && this.method !== 'CREATE') field.value = `'${field.value}'`;                     // In case of nonescaped field value and nonCREATE method - quote field value (for WHERE/INSERT clause)
      }
  ...
  switch (this.method)
         {
          case 'SELECT':
               ...
               return `SELECT ${this.Join(',', '')} FROM ${props.table}${this.Join(' AND ')}${props.orderasc || props.orderdesc ? ' ORDER BY ' + (props.orderasc || props.orderdesc) : ''}${'limit' in props ? ' LIMIT ' + props.limit : ''}`;
          case 'CREATE':
               ...
               if ('role' in props) return `CREATE ROLE ${props.role}; GRANT CONNECT ON DATABASE ${props.database} TO ${props.role}; GRANT USAGE ON SCHEMA public TO ${props.role}`; // CREATE ROLE <this.role>
               if ('database' in props) return `CREATE DATABASE ${props.database}`; // Create database
               if ('hypertable' in props) return `SELECT create_hypertable('${this.table}', by_range('${this.hypertable}'))`; // Create hyper table
          ...
          case 'WRITE':
               ...
               query = this.Join(' AND '); // Get WHERE clause in case of any field with sign does exist
               return query ? `UPDATE ${props.table} SET ${this.Join(',', '=')}${query}` : `INSERT INTO ${props.table} (${this.Join(',', '')}) VALUES (${this.Join('', ',')})`; // Update matched rows or insert new row otherwise
          case 'DROP':
               ...
               if ('database' in props) return `DROP DATABASE IF EXISTS ${this.database}`; // Drop database
               if (!props.fields.length) return `DROP TABLE IF EXISTS ${props.table}`; // Drop table for no any fields defined
               if ('index' in props) return `DROP INDEX IF EXISTS ${this.table}_${this.fields[0]}_index`; // Drop index for 1st field. Should 'DROP INDEX CONCURRENTLY IF EXISTS' be used?
               if (query = this.Join(' AND ')) return `DELETE FROM TABLE ${props.table}${query}`; // Drop rows matched WHERE clause
               ...
               return query;
          case 'SHOWTABLES':
               return this.BuildQuery({ method: 'SELECT', table: 'pg_tables', fields: ['tablename', { name: 'schemaname', sign: '=', value: 'public' }] });
          case 'BEGIN':
